...
int write_hevc_nal_unit(hevc_stream_t* h, uint8_t* buf, int size)
{
    ...
    bs_write_u(b, 6, nal->nuh_layer_id);
    bs_write_u(b, 3, nal->nuh_temporal_id_plus1);
...
void write_hevc_video_parameter_set_rbsp(hevc_stream_t* h, bs_t* b)
{
    ...
    /* vps_reserved_0xffff_16bits */ bs_write_u(b, 16, 0);
...
void write_hevc_slice_header(hevc_stream_t* h, bs_t* b)
{
    ...
    if( !sh->dependent_slice_segment_flag ) {
        for( i = 0; i < pps->num_extra_slice_header_bits; i++ ) {
            /* slice_reserved_flag */ bs_write_u(b, 1, 0);
        }
...
int read_debug_hevc_nal_unit(hevc_stream_t* h, uint8_t* buf, int size)
{
    ...
    printf("%ld.%d: ", b->p - b->start, b->bits_left); nal->nuh_layer_id = bs_read_u(b, 6); printf("nal->nuh_layer_id: %d \n", nal->nuh_layer_id); 
    printf("%ld.%d: ", b->p - b->start, b->bits_left); nal->nuh_temporal_id_plus1 = bs_read_u(b, 3); printf("nal->nuh_temporal_id_plus1: %d \n", nal->nuh_temporal_id_plus1); 
